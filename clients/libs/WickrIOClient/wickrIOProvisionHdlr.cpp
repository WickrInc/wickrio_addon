...
void WickrIOProvisionHdlr::registerWithPassword(const QString &newPassword)
{
    if (!newPassword.isEmpty() && (!m_email.isEmpty() || !m_username.isEmpty())) {
        ...
        if (m_mode == OnPremMode) {
            ...
            emit signalRegisterCloud(m_email, newPassword, m_transID, true, false, (m_mode == ForgotPasswordMode));  // Begins a process that eventually calls WickrIOProvisionHdlr::loginComplete()
        }
    }
}
