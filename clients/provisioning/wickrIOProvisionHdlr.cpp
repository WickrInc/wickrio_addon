...
void WickrIOProvisionHdlr::registerWithPassword(const QString &newPassword)
{
    if (!newPassword.isEmpty() && (!m_email.isEmpty() || !m_username.isEmpty())) {
        ...
        if (m_mode == OnPremMode) {
            ...
            signalRegisterOnPrem(m_username, m_password, newPassword, !m_userExists ? m_passwordSalt : QString(), m_transID, !m_userExists, m_userExists);
        } else if (m_mode == CloudMode || m_mode == ForgotPasswordMode) {
            m_currentStep = Step::loggingIn;
            signalRegisterEnterprise(m_email, newPassword, m_transID, true, false, (m_mode == ForgotPasswordMode));  // Begins a process that eventually calls WickrIOProvisionHdlr::loginComplete()
        }
    }
}
