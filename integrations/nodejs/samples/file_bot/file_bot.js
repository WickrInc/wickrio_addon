  ...
  async function listen(rMessage) {
    ...
    if (rMessage.message) {
      ...
      if (command === '/list') {
        ...
        try {
          var as = fs.statSync('files/' + attachment, fs.constants.R_OK | fs.constants.W_OK);
          console.log('as:', as)
        } catch (err) {
        ...
        console.log(__dirname + '/files/' + attachment)
        console.log(addon.cmdSendRoomAttachment(vGroupID, __dirname + '/files/' + attachment, attachment));
      ...
      try {
        fs.copyFile(rMessage.file.localfilename.toString(), 'files/' + rMessage.file.filename.toString(), (err) => {
          if (err) throw err;
        });
